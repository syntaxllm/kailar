...
export async function importMock({ force = false } = {}) {
  console.log("🛠️ importMock called with force =", force);
  const summary = { imported: 0, skipped: 0, chunks: 0, errors: [] };
  const files = (await fs.promises.readdir(mockDir)).filter(f => f.endsWith('.vtt'));
  console.log(`🔍 Found ${files.length} mock files in ${mockDir}`);
  ...
  for (const f of files) {
    try {
      ...
      console.log(`✅ Processed: ${f} (${chunks.length} chunks)`);
    } catch (err) {
