        ...
        try {
            const response = await fetch(`${BOT_SERVICE_URL}/join`, {
                ...
                body: JSON.stringify({
                    ...
                    disclosureMessage: disclosureMessage || "MeetingAI Bot is here to provide meeting insights."
                })
            });
            ...
            return { sessionId, status: BotStatus.ERROR, error: err.message };
        }
    }
...
/**
 * Get status of a bot session (including real-time speaker)
 */
export async function getBotSessionStatus(sessionId) {
    let baseStatus = { sessionId, status: BotStatus.IDLE, message: 'Bot not active' };

    if (BOT_SERVICE_URL) {
        try {
            const response = await fetch(`${BOT_SERVICE_URL}/status/${sessionId}`);
            if (response.ok) {
                baseStatus = await response.json();
            }
        } catch (err) {
    ...
    // Augment with real-time speaker from Redis
    const currentSpeaker = await redis.get(`speaker:${sessionId}`);
    if (currentSpeaker) {
        baseStatus.currentSpeaker = currentSpeaker.name;
    }
...
export async function getBotTranscript(sessionId) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                return {
                    ...
                    audioPath: data.audioPath // Capture audio path
                };
            }
        } catch (err) {
    ...
    return { sessionId, transcript: null, source: 'bot_stt', audioPath: null };
}
...
export async function handleBotWebhook(event) {
    ...
    switch (type) {
        ...
        case 'speaker_change':
            if (data?.name) {
                await redis.set(`speaker:${sessionId}`, { name: data.name }, 300);
            }
            return { acknowledged: true };

        case 'meeting_ended':
        case 'transcript_ready':
            return { acknowledged: true, shouldFetchTranscript: true };
