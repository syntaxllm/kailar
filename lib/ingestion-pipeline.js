...
function normalizeSTTTranscript(content) {
    ...
    // Common STT formats:
    // Whisper: { text: "...", segments: [{start, end, text}] }
    // Azure: { ... }
    ...
    if (data.segments) {
        ...
        return data.segments.map(seg => ({
            ...
            text: seg.text.trim(),
            speaker: seg.speaker || 'Unknown'  // Whisper can include diarization
        }));
    }

    // Fallback: assume array of entries
    if (Array.isArray(data)) {
        return data.map(entry => ({
            start: entry.start || '00:00:00',
