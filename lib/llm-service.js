 ...
 * Uses native MongoDB Atlas Vector Search
 */
export async function searchChunksSemantic(query, meetingId, limit = 10) {
    try {
        ...
        const { vectorSearchChunks } = await import('./storage-prod.js');

        // 1. Get the "Meaning" (Embedding)
        const queryVec = await generateEmbedding(query);

        // 2. Search Native DB (The real RAG)
        return await vectorSearchChunks(queryVec, meetingId, limit);
